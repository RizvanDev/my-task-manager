...
const Task = React.forwardRef((props, nodeRef) => {
  const { time, currentTask, tabTitle, completed, tasksMethods, timeLine, darkMode, children } =
    props

  const [taskValue, , taskOnChange] = useValue(children)
  const ref = useRef(null)

  const handleCompleteClick = () => tasksMethods.completeTask(tabTitle, currentTask, completed)
  ...
  const onKeyUpEdit = e => tasksMethods.editTask(e, tabTitle, currentTask, taskValue)

  const handleDeleteClick = () => tasksMethods.deleteTask(tabTitle, currentTask)

  return (
    <div className={darkMode ? 'task darkMode' : 'task'} ref={nodeRef}>
      <MainBtn
        type='button'
        style={{ display: timeLine.past || timeLine.future ? 'none' : 'flex' }}
        className={completed ? 'taskCheckbox checked' : 'taskCheckbox'}
      ...
      <textarea
        ...
        onKeyUp={onKeyUpEdit}
        className={completed ? 'taskName checked' : 'taskName'}></textarea>

      <MainBtn
        type='button'
        style={{ display: timeLine.past && 'none' }}
        className={completed ? 'taskEdit checked' : 'taskEdit'}
      ...
      <MainBtn
        type='button'
        style={{ display: timeLine.past && 'none' }}
        className={completed ? 'taskDelete checked' : 'taskDelete'}
